...
export default function GalleryPage() {
  return (
    <>
      ...
      {/* Gallery grid */}
      <section className="py-12 bg-cream" aria-label="Gallery">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[180px] md:auto-rows-[200px]">
            {galleryItems.map((item, i) => (
              <div
                key={item.id}
                className={`${item.span || ""} group relative overflow-hidden rounded-2xl border border-gold/10 bg-masala transition-all hover:border-gold/30 hover:shadow-md`}
                role="img"
              ...
              >
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-masala/92 via-masala/25 to-transparent" />
                ...
                <div className="relative z-10 flex h-full flex-col justify-between p-4">
                  <div className="flex items-start justify-between gap-3">
                    <span className="rounded-full border border-cream/30 bg-cream/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-cream">
                      {item.group}
                    </span>
                  ...
                  <div>
                    <div className="font-serif text-xl md:text-2xl font-bold leading-tight text-cream">
                      {item.label}
                    </div>
                    <div className="mt-2 text-xs uppercase tracking-[0.24em] text-cream/70">
      ...
      {/* Order CTA */}
      <section className="py-14 bg-cream-dark text-center px-4">
        <div className="premium-panel mx-auto max-w-xl px-8 py-8">
          <div className="premium-kicker mb-3 text-terracotta">Order Prompt</div>
          <h2 className="font-serif text-masala text-3xl md:text-4xl font-semibold mb-3">
            Hungry Already?
          </h2>
          <p className="text-masala/60 mb-6">
